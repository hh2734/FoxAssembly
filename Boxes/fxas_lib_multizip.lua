-- Error in library: fxas.lib.multizip >
-- FoxBox version: 2.13
-- fox.lua version: 1.1
-- Unix time: 1768652830
...
local Box = {}
Box._config = {
	package = "fxas.lib.multizip", -- Your package name
	version = 1.0, -- fxas.lib.multizip version
	deps = nil
}
--> ============   CONFIG   ============ <--
...
syslib.import("fxas.data.crc32")
local crc32 = fxas.data.crc32.crc32_byte
local bit = bit32
...
local inflate = Box.inflate


function Box.gunzip(t)
	local bs = get_bitstream(t.input)
	local outbs = get_obytestream(t.output)
...
function Box.inflate_zlib(t)
	...
	inflate{input=bs, output=
		disable_crc and outbs or
			function(byte)
				data_adler32 = Box.adler32(byte, data_adler32)
				outbs(byte)
			end
	}
