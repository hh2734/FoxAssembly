-- Error in library: fxas.crypto.norx >
-- FoxBox version: 2.13
-- fox.lua version: 1.1
-- Unix time: 1768656807
...
local Box = {}
Box._config = {
	package = "fxas.crypto.norx", -- Your package name
	version = 1.0, -- fxas.crypto.norx version
	deps = nil
}
--> ============   CONFIG   ============ <--
...
local function pad(ins)
	local out
	local inslen = #ins
	if inslen == 95 then return ins .. '\x81' end
...
local function decrypt_lastblock(s, out_table, last)
	...
	local lastblock_s8_table = {}
	for i = 1, 12 do
		local s8 = spack("<I8", s[i])
		insert(lastblock_s8_table, s8)
	end
	local lastblock = concat(lastblock_s8_table)
